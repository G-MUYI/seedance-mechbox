...
2. **开头必须立刻有动作**：按钮下沉、红光亮起、魔盒坠落或撞桌必须马上发生，不做片头、不做解释、不做 logo 展示。
...
8. **循环感**：结尾保持到位后的轻微手持停留，Caption 可引导重看机械展开细节。
...
- 不要丢失时间轴中的所有阶段：`触发与下坠`、`落桌碰撞`、`严格串行机械展开`、`展开段一` 到 `展开段六`、`锁扣与小件收束`、`终态稳定停留`。
...
4. 初始展示阶段只显示由目标物反推得到的封闭初始机械实体，不提前露出目标物完整形态。
5. 主体展开阶段严格串行机械展开，任意时刻只允许一个部件组动作。
...
【反剧透要求】在初始展示阶段，盒体保持纯硬边工业机械盒外观，{Target Name} 的完整可识别形态只能以内部结构数量、分缝节奏、铰链分布、前端源开口位置和承重支点布局存在，不以完整轮廓提前呈现。

五、全局动作语义

整条视频的机械动作气质统一表现为：高延迟、重阻尼、强摩擦、阶段性停滞、轻微错齿、传动迟滞、沉重反馈、逐级到位、最终扣合。任何主要部件组启动时，先出现轻微抖动与受力迟疑，再进入推进过程。主要部件组遵循五拍微循环：启动轻抖 → 缓慢推进到约 60% → 短暂停滞 → 回退约 5% → 再次推进 → 最终到位；小件只保留必要的停滞、扣合和到位反馈。每个主要部件组完成后都有明确金属反馈与扣合反馈，然后下一个部件组才开始。
...
阶段二：落桌碰撞
魔盒砸到桌面，底面边角先接触桌面，发生真实碰撞，轻微弹跳一次后稳定停住，接触阴影压实，桌面反光发生短促变化。撞击瞬间镜头出现一次很轻的反应性震动，震动在同一连续镜头内自然衰减，继续保持手持观察状态。
...
展开段一：顶盖主齿轮分级转动，带动外壳沿 {主要分缝} 裂开，裂开方式体现 {系列气质}。
...
阶段五：终态稳定停留
完整 {Target Name} 保持清楚可见，没有半展开面板、没有卡住的套筒、没有缺失关键部件；承重点与桌面真实接触，接触阴影稳定，镜头只做轻微呼吸式停留。
...
【机械盒反向设计】
初始魔盒为{根据目标物形态确定的盒子形状}，暗示最终{Target Name}轮廓；{主要部件源开口布局}；触发按钮与{目标主题相关}；正面齿轮窗预显{目标特征}。

主体：这个金属机械盒从触发→下落→落桌→严格串行展开成{Target Name}。
...
- Prompt 中初始展示阶段只像一个由咖啡杯反推的封闭机械杯状实体，不提前出现完整咖啡机。
